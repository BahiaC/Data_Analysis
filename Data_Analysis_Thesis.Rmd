...
ggplot(phyla_plot, aes(x=Phylum, y=Abundance, colour=Phylum, position="dodge"))+geom_bar(stat="identity")+facet_grid(.~Participant)+scale_colour_manual(values=c("red2", "orange1", "yellow1", "limegreen", "deepskyblue2","purple1", "hotpink"), labels=c("Actinobacteria", "Bacteroidetes", "Firmicutes", "Proteobacteria", "Synergistetes", "TM7", "Verrucomicrobia"))
...
##PreCon BetaDiv
plot_ordination(pre_con_rare_fil,pre_con_ord, color="Participant") + stat_ellipse()+labs(title="Weighted Unifrac PCoA Baseline Depressed vs Controls")+scale_color_manual(values=c("limegreen", "red2"), labels=c("Controls", "Baseline Depressed")) 

##PreCon DepDass Roseburia 
depdass_roseburia<-ggplot(roseburia_counts_DepDass, aes(x=Abundance, y=DEP_DASS))+geom_point(aes(color=Participant)) + labs(title="DEP_Dass Correlation to Rosburia 518438 in Baseline \nDepressed and Control Samples")+geom_smooth(method=lm)

depdass_roseburia + scale_colour_manual(values=c("green","red"), labels=c("Controls", "Baseline Depressed"))

##SevereMild Ruminoc
depression_smelt_rare_counts %>% mutate(meanAbund= mean(Abundance)) %>% filter(OTU=="295748") %>% filter(PRE_BDI_LEVELS!=("NA")) %>% filter(PRE_BDI_LEVELS!=("moderate")) %>%ggplot(aes(x=PRE_BDI_LEVELS, y=Abundance))+ geom_point()+labs(title="Abundance of OTU 295748 f_Ruminococcaceae in Severe &\nMild BDI Baseline Depressed Samples", x="BDI Severity Level", fill="BDI Severity Level")+geom_boxplot()+scale_x_discrete(labels=c("Mild", "Severe"))
...
allocation_a<-subset_samples(depression2017_fil, Allocation_Groups %in% c("PreA","PostA"))
allocation_a_ord<-ordinate(allocation_a, method="PCoA", "wunifrac")
plot_ordination(allocation_a, allocation_a_ord, color="Allocation_Groups")+stat_ellipse()+labs(title="Weighted Unifrac PCoA Pre and Post Depressed Participant \nSamples Probiotic Treatment", color="Probiotics")+scale_color_manual(values=c("orange", "deepskyblue2"), labels=c("Post Probiotics", "Pre Probiotics"))

allocation_b<-subset_samples(depression2017_fil, Allocation_Groups %in% c("PreB","PostB"))
allocation_b_ord<-ordinate(allocation_b, method="PCoA", "wunifrac")
plot_ordination(allocation_b, allocation_b_ord, color="Allocation_Groups")+stat_ellipse()+labs(title="Weighted Unifrac PCoA Pre and Post Depressed Participant \nSamples Placebo Treatment", color="Placebo")+scale_color_manual(values=c("orange", "deepskyblue2"), labels=c("Post Placebo", "Pre Placebo"))


##Top100 Dep_Dass Severity Correlation
top_100_gnavus$PRE_BDI_LEVELS<-factor(top_100_gnavus$PRE_BDI_LEVELS, levels=c("mild", "moderate", "severe", "Con"))

ggplot(top_100_gnavus, aes(x=Abundance, y=DEP_DASS))+geom_point(aes(colour=PRE_BDI_LEVELS)) + labs(title="DEP_DASS correlation with R. gnavus 360015", colour="BDI Severity Levels")+geom_smooth(method=lm)+scale_colour_manual(values=c("dodgerblue1","orange1","red2","limegreen"), labels=c("Mild", "Moderate", "Severe", "Control"))


##Probiotics 
ggplot(probiotics, aes(x=Participant_Group, y=sum_abund, fill=Allocation_Groups))+geom_bar(stat="identity", position="dodge")+labs(title="Pre and Post Probiotic Treatment Baseline\nDepressed Participants Probiotic Levels", y="% Relative Abundance", x="Sample",fill="Pre and Post")+theme(axis.text.x=element_text(angle=90, size=8))+facet_grid(.~Genus)+coord_fixed(ratio=155, expand = TRUE)+scale_fill_manual(values=c("deepskyblue3","red2"), labels=c("Pre Probiotic","Post Probiotic"))

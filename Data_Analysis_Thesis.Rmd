...
##SevereMild Ruminoc
severemild_rumino<-depression_smelt_rare %>% mutate(meanAbund= mean(Abundance)) %>% filter(OTU=="295748") %>% mutate(stddev= sd(Abundance)) %>%filter(PRE_BDI_LEVELS!=("NA")) %>% filter(PRE_BDI_LEVELS!=("moderate"))%>% filter(PRE_BDI_LEVELS!="Con")
...
severemild<-depression_smelt_rare %>% filter(Participant=="Pre") %>% filter(Severe_Mild %in% c("severe", "mild")) %>% filter(OTU=="295748") %>% group_by(Severe_Mild, OTU) %>% mutate(meanAbund= mean(Abundance)) %>% mutate(stddev= sd(Abundance))

severemild_fil<-depression_smelt %>% filter(Participant=="Pre") %>% filter(Severe_Mild %in% c("severe", "mild")) %>% filter(OTU=="295748") %>% group_by(Severe_Mild, OTU) %>% mutate(meanAbund= mean(Abundance)) %>% mutate(stddev= sd(Abundance))

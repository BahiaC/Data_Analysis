...
severemild<-depression_smelt_1 %>% filter(Participant=="Pre") %>% filter(Severe_Mild %in% c("severe", "mild")) %>% filter(OTU=="295748") %>% group_by(Severe_Mild, OTU) %>% summarise(meanAbund=mean(Abundance), sd=sd(Abundance))
...
data<-data.frame(sample_data(depression2017)) %>% filter(Allocation_Groups %in% c("PreA", "PreB", "PostA", "PostB")) %>% select(BDI, Allocation, Participant)
melt(data,"BDI")
```

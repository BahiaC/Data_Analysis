...
probiotics<- depression_smelt_rare %>% filter(Genus %in% c("g__Bifidobacterium","g__Lactobacillus", "g__Lacotococcus")) %>% group_by(sample_Sample, Genus) %>% mutate(sum_abund= sum(Abundance)) %>% select(OTU,Sample,Abundance,Participant,ProA_B,Pre_ProA,Pre_PostB,Genus,sum_abund) %>%
  filter(Participant %in% c("Pre","Post"))
...
ggplot(top_100_gnavus, aes(x=OTU, y=Abundance, fill=Participant)) %>% +geom_bar(stat="identity", position="dodge")

ggplot(top_100_gnavus, aes(x=OTU, y=Abundance, fill=Participant)) %>% +geom_boxplot()


ggplot(top_100_gnavus, aes(x=Abundance, y=DEP_DASS))+geom_point(aes(colour=Participant)) + labs(title="DEP_DASS correlation with R. gnavus 360015")+geom_smooth(method=lm)
